...
void scoreTask() {
	...
	{
		struct ScoreMessage *msg = (struct ScoreMessage *) WaitPort(taskPort);
...
int main() {
	...
	struct ScoreMessage *resetMsg = AllocMem(sizeof(struct ScoreMessage), MEMF_PUBLIC | MEMF_CLEAR);
	...
	PutMsg(taskPort, (struct Message *) resetMsg);

	struct ScoreMessage *set1Msg = AllocMem(sizeof(struct ScoreMessage), MEMF_PUBLIC | MEMF_CLEAR);
	...
	set1Msg->playerno = 1;
	set1Msg->score = "     00";
	PutMsg(taskPort, (struct Message *) set1Msg);

	struct ScoreMessage *set2Msg = AllocMem(sizeof(struct ScoreMessage), MEMF_PUBLIC | MEMF_CLEAR);
	...
	set2Msg->playerno = 2;
	set2Msg->score = "    400";
	PutMsg(taskPort, (struct Message *) set2Msg);

	for (i = 0; i < 240; i++) {
		struct ScoreMessage *addMsg = AllocMem(sizeof(struct ScoreMessage),
		...
		addMsg->playerno = 1;
		addMsg->score = "     10";
		PutMsg(taskPort, (struct Message *) addMsg);
		Delay(2);
	}
...
bye:
	if (task) {
		...
		PutMsg(taskPort, (struct Message *) &stop);
		Delay(10);
	}
