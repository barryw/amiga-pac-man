...
int main() {
    ...
    leftDevice = OpenDevice(AUDIONAME, 0L, (struct IORequest *) AudioLeft, 0L);
    if (leftDevice != 0) {
    ...
    rightDevice = OpenDevice(AUDIONAME, 0L, (struct IORequest *) AudioRight, 0L);
    if (rightDevice != 0) {
    ...
    AudioLeft->ioa_Data = coindrop_raw;
    AudioLeft->ioa_Length = coindrop_raw_len;
    AudioLeft->ioa_Period = clock / 22050;
    ...
    AudioRight->ioa_Data = coindrop_raw;
    AudioRight->ioa_Length = coindrop_raw_len;
    AudioRight->ioa_Period = clock / 22050;
